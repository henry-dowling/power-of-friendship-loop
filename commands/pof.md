...
4. Otherwise, run `python -m pof $ARGUMENTS`. Preserve supported flags such as `--max-turns`, `--from`, `--agent`, `--dry-run`, and `--headless`. Treat `--iterations` as a compatibility alias for `--max-turns`. For plain natural-language goal text, pass it as one shell-quoted argument.
