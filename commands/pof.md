...
4. Otherwise, run `python -m pof $ARGUMENTS`. Preserve supported flags such as `--iterations`, `--from`, `--agent`, and `--dry-run`. For plain natural-language goal text, pass it as one shell-quoted argument.
5. Report whether the loop completed, hit the iteration limit, or failed, and include the transcript path when one is printed.
